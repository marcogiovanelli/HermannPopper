Pauli's point is that \emph{classical and quantum mechanics differ not merely in the attainable precision of prediction, but in the epistemic role of successive measurements}. The epistemic role of successive measurements. In classical mechanics, measurements can accumulate information; in quantum mechanics, a new measurement can destroy the predictive value of information obtained from earlier measurements.

Considers the trajectory of a planet. Suppose we repeatedly measure its position at times

\begin{equation*}
t_0,t_1,t_2,\ldots
\end{equation*}

and every individual measurement has the same finite uncertainty $\Delta x_0$.

At first sight, one might think that this finite experimental accuracy imposes a permanent limit on our knowledge of the orbit. But this is not so in classical mechanics. If we assume a relatively simple deterministic law of motion, such as Newtonian gravitation, then all the measurements constrain \emph{one and the same underlying trajectory}.

Consequently, many relatively imprecise observations can collectively determine that trajectory much more precisely than any single observation. Once the dynamical law and the trajectory have been inferred, one can calculate both position and velocity at arbitrary times with an accuracy that can, in principle, become arbitrarily high as the observational evidence increases.

Schematically:

\begin{equation*}
\text{many measurements of limited precision}
\quad\Longrightarrow\quad
\text{increasingly precise knowledge of one trajectory}.
\end{equation*}

This inference depends on an idealization characteristic of classical mechanics: one assumes that the body follows a smooth trajectory governed by a simple force law rather than undergoing unknown microscopic zigzag motions between observations.

In classical mechanics, measurements can accumulate information; in quantum mechanics, a new measurement can destroy the predictive value of information obtained from earlier measurements.



\section*{2. Quantum mechanics: measurements do not simply accumulate}

Pauli then argues that the analogous procedure fails in quantum mechanics.

Suppose again that we repeatedly measure position, each time with uncertainty $\Delta x_0$. A position measurement of this accuracy necessarily introduces a corresponding uncertainty in momentum. Schematically, from the uncertainty relation,

\begin{equation*}
\Delta p \sim \frac{h}{\Delta x_0}.
\end{equation*}
...
this means an uncertainty in velocity of approximately

\begin{equation*}
\Delta v\sim\frac{h}{m\Delta x_0}.
\end{equation*}

During the interval $t_{n+1}-t_n$, that velocity uncertainty produces an uncertainty in the subsequent position of order

\begin{equation*}
\Delta x_{n+1}
\sim
\frac{h}{m\Delta x_0}(t_{n+1}-t_n).
\end{equation*}

The crucial point is that \emph{each new position measurement resets the relevant quantum state}. It does not merely add another datum to a preexisting set of measurements describing an independently existing trajectory. By localizing the particle anew, it changes the conditions governing subsequent predictions.

Hence measurements at $t_0,t_1,\ldots,t_{n-1}$ cannot simply be pooled with the measurement at $t_n$ to reconstruct an increasingly precise underlying trajectory. Once the measurement at $t_n$ has been made, earlier information can become irrelevant for predicting what will happen after $t_n$, within the uncertainty introduced by that latest measurement.

\section*{3. Pauli's central claim}

This is what Pauli regards as the \q{Hauptunterschied} between the two theories: in classical mechanics, additional measurements normally \emph{increase} one's stock of usable information; in quantum mechanics, a new measurement can cause information obtained from previous measurements to \emph{lose its predictive relevance}.

This is stronger than the familiar statement that quantum mechanics imposes limits on simultaneous measurement precision. Pauli is emphasizing the \emph{temporal structure of measurement and prediction}.

Classically, one can think:

\begin{equation*}
\text{past measurements}
\longrightarrow
\text{better reconstruction of the trajectory}
\longrightarrow
\text{better future predictions}.
\end{equation*}

Quantum mechanically, by contrast:

\begin{equation*}
\text{past measurements}
\longrightarrow
\text{state at }t_n
\xrightarrow{\text{new measurement}}
\text{new state}
\longrightarrow
\text{future probabilities}.
\end{equation*}

The new measurement is therefore not epistemically passive. It alters which previously acquired information remains usable for subsequent prediction.

\section*{4. Why the contrast matters conceptually}

Pauli is targeting the classical idea that there exists a continuously defined trajectory whose properties are merely imperfectly revealed by measurements. If that picture remained valid in quantum mechanics, one might hope that sufficiently many coarse measurements could be combined statistically to reconstruct the trajectory with arbitrary precision, just as in the planetary example.

His argument is that quantum mechanics blocks precisely this move. The limitation represented by the uncertainty relations cannot be overcome simply by \emph{repeating measurements and pooling their results}, because the measurements themselves successively establish new conditions for prediction.


Thus Pauli's complaint to Born in the final sentence is quite precise: Born's manuscript apparently presented the difference between classical and quantum mechanics primarily as a difference concerning uncertainty or predictability, whereas Pauli wanted him to emphasize a more radical point:

\begin{quote}
Classical uncertainty can, in principle, be progressively reduced by accumulating observations of the same dynamical history. Quantum uncertainty cannot generally be reduced in this way because successive measurements do not merely reveal that history; they can erase the predictive significance of information supplied by earlier measurements.
\end{quote}

This is also why Pauli's example is closely connected with Bohr's conception of measurement. The relevant contrast is not simply \emph{determinism versus indeterminism}, but between a classical conception in which observations can be treated as accumulating information about an independently evolving trajectory and a quantum conception in which the experimental intervention itself helps determine the physical conditions under which subsequent predictions are meaningful.


Hermann emphasizes the logical/contextual point: mutually exclusive possible determinations cannot be combined. Pauli emphasizes its temporal or dynamical consequence: successive measurements therefore cannot function as cumulative evidence for an increasingly precise trajectory.


In classical mechanics, suppose we have position measurements at $t_0,t_1,t_2,\ldots$. Each pair of measurements contributes information about the same continuous past trajectory. As further measurements are added, the reconstruction becomes progressively more precise. Thus the reconstructed segments are cumulative

Hence the information furnished by earlier measurements cannot simply be carried forward and combined with later measurements to produce an indefinitely refined trajectory.

successive determinations cannot be accumulated into an increasingly precise trajectory.

Yes. In Pauli’s example he deliberately chooses the relatively favorable case of a sequence of measurements of the same observable, position. Even there, classical cumulativity fails. If one alternates position and momentum measurements, the failure becomes much more conspicuous.

The information is cumulative

He shows that even repeated measurements of one and the same quantity fail to have the cumulative character they possess classically,

%And this is exactly why Pauli's example with repeated position measurements is conceptually powerful. He derives diachronic noncumulativity from synchronic incompatibility. Even though every actual measurement in his example is a position measurement,


%Pauli's point is that classical and quantum mechanics differ not merely in the attainable precision of prediction, but in the epistemic role of successive measurements. In classical mechanics, measurements can accumulate information; in quantum mechanics, a new measurement can destroy the predictive value of information obtained from earlier measurements.
...
%\q{the $\psi$-function, which you rely on, is inappropriate to the problem you intend to deal with. Although it represents a solution of the Schroedinger equation and fulfils the boundary conditions, it does not satisfy the initial conditions. Indeed, it lacks, in your own words, the properties required for the description of an individual system. But other solutions produced by superposition exist which satisfy the initial conditions required when one wants to follow an individual system. This can, of course, only be done approximately, but the greater the mass $m$, the more accurate it is. I have just calculated this limiting transition $m \rightarrow \infty$, by using your example, and have found that it leads exactly to the classical description. The calculation is free from difficulty, and has been confirmed not only by my collaborators, but also by my successor, Prof. Kemmer, and by the critical and sceptical Schroedinger. If you are in any doubt, ask Johann V. Neumann to read the manuscript, or Weyl, who is probably now in Princeton.}



%You were probably affronted because I used the opportunity to hit out at classical determinism. But I am convinced that even this will eventually seem plausible to you too, if you take plenty of time to read it, and discuss it with Weyland Neumann. In any case, you must not be angry with me. 


%In this situation it was fortunate that a third person intervened and acted as intermediary: Wolfgang Pauli. I have included three of his letters to me below. He has already appeared in this correspondence as my assis- tant in GOttingen, the first of a series of outstanding young people. He had then, when barely twenty years old, already written a great work, the article about the theory of relativity in the Encyclopaedia, which was for a long time the best representation of the theory of relativity and is today still regarded as one of the authoritative sources. Pauli became a professor in Zurich. He went to America during the second world war, as he feared that Switzerland, like other small countries such as Belgium, Holland and Norway, would also be overrun by Hitler's armies. He became a close friend of Einstein's and regarded himself, probably with some justification, as the designated 'successor' in theoretical physics. With me, too, he remained in constant contact, although mainly by lette




%Born's counterexample misses Einstein's point. He presents a case of a macroscopic system that is initially localized and shows that it can be described within \qm. But Einstein's objection is not that \qm\ cannot represent localized macroscopic bodies. The decisive consequence is that, if all solutions of the Schrödinger equation are physically realizable, then macroscopic superpositions must likewise be physically realizable. A macroscopic body need not be in a quasi-sharp position prior to observation; rather, it may be represented by a $\psi$-function spread over macroscopically distinct positions. On the completeness interpretation, this would mean that the body has no definite position until it is observed, for instance by being illuminated. Einstein's objection is that this conclusion dissolves the definite macroscopic world into the very \s{fog} that the orthodox interpretation was supposed to avoid.

%* Born: There exists a subclass of solutions, selected by suitable initial conditions, whose evolution remains quasi-classical. Hence the theory is compatible with ordinary macroscopic experience.
%* Einstein: That is irrelevant. Unless you can provide a physical principle excluding the other solutions, the theory predicts that macroscopic systems may also occupy superpositions of macroscopically distinct positions. If the wave function is complete, these states correspond to genuine physical possibilities.


%Born, M.
%\begin{itemize}
%\item[(1949)] The foundations of quantum statistics. Nuovo Cimento 6 (Supplemento) 161-166 (1949)
%\item[] (1953a) The conceptual situation in physics and the prospects of its future development. Proc. Phys. Soc. A 66, 501-513 (1953). Eine auszugsweise Übersetzung erschien auch in Phys. B1. 10, 193-201 (1954)
%\item[] (1953b) Physical reality. Philosophical Quarterly 3, 139-149 (1953)
%\item[(1954)] Physikalische Wirklichkeit. Phys. B1. 10, 49-61 (1954)
%\item[] (1955a) Continuity, determinism, and reality. Kong. Danske Vid. Selsk. 30, Nr. 2, 1-26 (1955)
%\item[] (1955b) Albert Einstein und das Lichtquantum. Naturwiss. 42, 425-431 (1955)
%\end{itemize}

%It is true that, once initial uncertainties are given, classical and quantum mechanics may display similar later growth of uncertainty. But this does not show that quantum mechanics captures the individual case in the same sense as classical mechanics. The specifically quantum feature is the existence of a minimum uncertainty, imposed by the commutation relations. This step may be interpreted, as Born wishes, as the precise quantitative formulation of a limitation already present in classical physics; but it remains a substantive hypothesis, not a logical necessity. Einstein is therefore not simply confused when he resists it. He tries to preserve the concept of the individual process and for that reason interprets quantum mechanics only statistically. Schrödinger accepts neither Born’s view nor Einstein’s, but insists that the unresolved conceptual difficulty is real

%\footnote{Zwei Traume 1. Ich bin in einem frisch ausbetonierten Schacht. Meine Frau kommt spater auch. Unten treffe ich Einstein, der mir wichtige Formeln iiber Kriimmung und die flinfte Dimension angibt. Diese bin ich im Begriffe, auch anderen zu erzahlen, als ich erwache 15. Februar [1954]}
